...
contract iQrad_V1 is Ownable {
    ...
    function _takeLoan(
        ...
        require(amount >= minLoanAmount, "Loan amount is less than minimum required for the duration");
        ...
        IERC20(islamiToken).transfer(deadWallet, serviceFee);
        ...
        users[user].nextPaymentTime = block.timestamp + oneMonth;
        ...
        returns (address, uint256)
    {
        ...
        for (uint256 i = 0; i < investors.length; i++) {
            ...
            for (uint256 vaultId = 1; vaultId <= vaultCount; vaultId++) {
                ...
                if (
                    vault.availableAmount >= loanAmount &&
    ...
    function makeMonthlyPayment() external hasActiveLoan(msg.sender) {
        ...
        user.nextPaymentTime = block.timestamp + oneMonth;
    ...
    function handleDefault(address _user) public hasActiveLoan(_user) {
        ...
        uint256 iGoldNeeded = (user.monthlyFee * (1e8)) / uint256(iGoldPrice);
